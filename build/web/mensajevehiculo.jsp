<%-- 
    Document   : mensajevehiculo
    Created on : 25/05/2020, 04:22:43 PM
    Author     : Mindchift
--%>
...
<%
    if (request.getParameter("regvehiculo") != null) {
        if (request.getParameter("placa") != ""
            ...
            if (v.buscar(request.getParameter("placa"))) {
%>
        <script> 
            document.getElementById("titulo").innerHTML = "El vehiculo ya existe, registrar uno nuevo.";
        </script>
<%
            } else {
                String valor = "";
                ...
                rs = consulta.executeQuery("select VALOR_COMERCIAL from tabla where MARCA='"+request.getParameter("marca")+"' and MODELO='"+request.getParameter("modelo")+"'");
                if(rs.next()){
                    valor = rs.getString("VALOR_COMERCIAL");
                }
                v.nuevo(request.getParameter("placa"), request.getParameter("modelo"), request.getParameter("marca"),
                        request.getParameter("serial"), valor, request.getParameter("propietario"));
%>
        <script>
            document.getElementById("titulo").innerHTML = "Vehiculo agregado correctamente";
            var texto2 = document.getElementById("menper");
            let params = new URLSearchParams(location.search);
            var contract = params.get('propietario');
            var contract2 = params.get('placa');
...
<script>
    ...
    function red2() {
        location.href = "registrarvehiculo.jsp";
    }
    window.onload = dos;
</script>
